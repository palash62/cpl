...
echo "       bash $ROOT/deploy/start-email-worker.sh"
echo "     One-time boot persistence: pm2 startup   # follow printed systemd command"
echo "  7. Re-queue stuck sends (loop until clear — 500 rows per run):"
echo "       bash $ROOT/deploy/reconcile-email-cron.sh"
echo "     Or install safety-net cron:"
echo "       */15 * * * * $ROOT/deploy/reconcile-email-cron.sh >> /var/log/cpl-reconcile.log 2>&1"
echo ""
