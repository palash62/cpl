...
export function AdvertiserLeadDetailsFilters({ campaigns }: { campaigns: CampaignOption[] }) {
  ...
  const selectedCampaign =
    campaignId === "all"
      ? "All campaigns"
      : campaigns.find((c) => c.id === campaignId)?.name ?? "All campaigns";

  const selectedStatus = STATUSES.find((s) => s.value === status)?.label ?? "All statuses";
    ...
    <div className="border-b border-slate-100 bg-slate-50/80 px-4 py-2.5">
      <div className="flex w-full flex-wrap items-center gap-2">
        <div className="min-w-[200px] flex-1">
          <Select
            ...
            onValueChange={(v) => {
              const next = v ?? "all";
              setCampaignId(next);
              applyFilters({ campaignId: next });
            }}
          >
            <SelectTrigger className={SELECT_TRIGGER_CLASS}>
              <SelectValue placeholder="All campaigns">{selectedCampaign}</SelectValue>
            </SelectTrigger>
        ...
        <div className="min-w-[160px]">
          ...
          >
            <SelectTrigger className={SELECT_TRIGGER_CLASS}>
              <SelectValue placeholder="All statuses">{selectedStatus}</SelectValue>
            </SelectTrigger>
            <SelectContent align="start" alignItemWithTrigger={false} className={SELECT_MENU_CLASS}>
              {STATUSES.map((s) => (
