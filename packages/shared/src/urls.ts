...
export function buildSmartLinkUrl(slug: string, params?: { src?: string; subId?: string }) {
  const base = `${getTrackingUrl()}/s/${slug}`;
  return params ? buildTrackingUrl(base, params) : base;
}

export function buildTrackingFormUrl(slug: string, params?: { src?: string; subId?: string }) {
  const base = `${getTrackingUrl()}/t/${slug}`;
  return params ? buildTrackingUrl(base, params) : base;
}

export function buildPixelUrl(pixelToken: string) {
  const base = `${getTrackingUrl()}/api/v1/pixel/${pixelToken}`;
